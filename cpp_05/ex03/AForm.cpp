...
AForm::AForm()
    : name("Default"), sign_status(false), req_grade_sign(150),
      req_grade_exec(150)
{
...
AForm::AForm(const AForm & src)
    : name(src.name), sign_status(src.sign_status),
      req_grade_sign(src.req_grade_sign), req_grade_exec(src.req_grade_exec)
{
    debug_log("copy constructor called");
}

AForm::AForm(std::string const name, unsigned short int req_sign,
             unsigned short int req_exec)
    : name(name), sign_status(false)
{
...
AForm::~AForm() { debug_log("destructor called"); }
...
std::ostream & operator<<(std::ostream & os, const AForm & rhs)
{
    ...
    os << "Required Grade to Execute: " << rhs.getReqGradeExec() << std::endl;
...
unsigned short int AForm::getReqGradeSign() const { return req_grade_sign; }
unsigned short int AForm::getReqGradeExec() const { return req_grade_exec; }
bool AForm::getSignStatus() const                 { return sign_status; }
std::string const & AForm::getTarget() const      { return target; }

void AForm::setTarget(std::string const target)   { this->target = target; }
